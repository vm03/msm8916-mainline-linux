...
static struct numa_group *deref_task_numa_group(struct task_struct *p)
{
	return rcu_dereference_check(p->numa_group, p == current ||
		(lockdep_is_held(__rq_lockp(task_rq(p))) && !READ_ONCE(per_task(p, on_cpu))));
}
	...
	for_each_cpu(cpu, cpumask_of_node(nid)) {
		...
		if (find_idle && !rq->nr_running && idle_cpu(cpu)) {
			if (READ_ONCE(rq->numa_migrate_on) ||
			    !cpumask_test_cpu(cpu, per_task(env->p, cpus_ptr)))
				continue;
			      ...
			      long taskimp, long groupimp, bool maymove)
{
	...
	if (!cpumask_test_cpu(env->src_cpu, per_task(cur, cpus_ptr)))
		goto unlock;
				...
				long taskimp, long groupimp)
{
	...
	for_each_cpu(cpu, cpumask_of_node(env->dst_nid)) {
		...
		if (!cpumask_test_cpu(cpu, per_task(env->p, cpus_ptr)))
			continue;
...
find_idlest_group_cpu(struct sched_group *group, struct task_struct *p, int this_cpu)
{
	...
	for_each_cpu_and(i, sched_group_span(group), per_task(p, cpus_ptr)) {
				  ...
				  int cpu, int prev_cpu, int sd_flag)
{
	...
	if (!cpumask_intersects(sched_domain_span(sd), per_task(p, cpus_ptr)))
		return prev_cpu;
...
static int select_idle_core(struct task_struct *p, int core, struct cpumask *cpus, int *idle_cpu)
{
	...
	for_each_cpu(cpu, cpu_smt_mask(core)) {
		if (!available_idle_cpu(cpu)) {
			idle = false;
			if (*idle_cpu == -1) {
				if (sched_idle_cpu(cpu) && cpumask_test_cpu(cpu, per_task(p, cpus_ptr))) {
					*idle_cpu = cpu;
					break;
				}
				continue;
		...
		if (*idle_cpu == -1 && cpumask_test_cpu(cpu, per_task(p, cpus_ptr)))
			*idle_cpu = cpu;
	}
...
static int select_idle_cpu(struct task_struct *p, struct sched_domain *sd, bool has_idle_core, int target)
{
	...
	cpumask_and(cpus, sched_domain_span(sd), per_task(p, cpus_ptr));
...
static int select_idle_sibling(struct task_struct *p, int prev, int target)
{
	...
	recent_used_cpu = per_task(p, recent_used_cpu);
	per_task(p, recent_used_cpu) = prev;

	if (recent_used_cpu != prev &&
	    recent_used_cpu != target &&
	    cpus_share_cache(recent_used_cpu, target) &&
	    (available_idle_cpu(recent_used_cpu) || sched_idle_cpu(recent_used_cpu)) &&
	    cpumask_test_cpu(per_task(p, recent_used_cpu), per_task(p, cpus_ptr)) &&
...
EXPORT_SYMBOL_GPL(em_cpu_energy);
#endif
...
int can_migrate_task(struct task_struct *p, struct lb_env *env)
{
	...
	if (!cpumask_test_cpu(env->dst_cpu, per_task(p, cpus_ptr))) {
		int cpu;
...
more_balance:
	...
	if (!ld_moved) {
		...
		if (need_active_balance(&env)) {
			 ...
			 * moved to this_cpu:
			 */
			if (!cpumask_test_cpu(this_cpu, per_task(busiest->curr, cpus_ptr))) {
